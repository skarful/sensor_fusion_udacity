...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
	...
	viewer->setBackgroundColor (0, 0, 0);
  	...
  	//Change view colors
  	// viewer->setShapeRenderingProperties ( pcl::visualization::PCL_VISUALIZER_COLOR, 0.5, 0.5, 0.5, "window" );
...
void proximityFunction(const int id,
  ...
  for (const int query_id : nearby_ids) {
    ...
    if (!processed[query_id]) {
      proximityFunction(query_id, points, tree, cluster, processed,
                              distanceTol);
    }
  }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
  ...
  for (size_t i = 0; i < points.size(); i++) {
    ...
    proximityFunction(i, points, tree, cluster, processed, distanceTol);
